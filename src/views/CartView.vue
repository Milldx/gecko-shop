...
import { goods, cart, removeFromCart, updateCartQty, clearCart, token, saveOrder, applyPromo } from '../composebles/useGoods.js'
...
const loyaltyProgress = computed(function() {
  if (loyaltyLevel.value === 'PLATINUM') return { pct: 100, next: null, need: 0 }
  if (loyaltyLevel.value === 'GOLD') return { pct: Math.round((totalSpent.value - 50000) / 500), next: 'PLATINUM', need: 100000 - totalSpent.value }
  if (loyaltyLevel.value === 'SILVER') return { pct: Math.round((totalSpent.value - 20000) / 300), next: 'GOLD', need: 50000 - totalSpent.value }
  return { pct: Math.round(totalSpent.value / 200), next: 'SILVER', need: 20000 - totalSpent.value }
})
...
// Цена за единицу товара с учётом sale-скидки
...
const totalPrice = computed(function() {
  let sum = 0
  for (let cartIndex = 0; cartIndex < cartItems.value.length; cartIndex++) {
    sum = sum + effectivePrice(cartItems.value[cartIndex].product) * cartItems.value[cartIndex].qty
  }
  return sum
})
...
const promoApplied = ref('')

function checkPromo() {
  if (!token.value) return
  const result = applyPromo(promoInput.value)
  if (result === null) {
    promoError.value = 'Промокод не найден или неактивен'
    promoDiscount.value = 0
    promoApplied.value = ''
  } else {
    promoError.value = ''
    promoDiscount.value = result
    promoApplied.value = promoInput.value.trim().toUpperCase()
  }
}
...
// Сумма скидки по промокоду (считается от цены после лояльности)
const promoDiscountAmount = computed(function() {
  if (!promoDiscount.value) return 0
  return Math.round((totalPrice.value - discountAmount.value) * promoDiscount.value / 100)
})

// Цена после скидок
const priceAfterDiscount = computed(function() {
  return totalPrice.value - discountAmount.value - promoDiscountAmount.value
})
...
const checkoutDeliveryCost = computed(function() {
  ...
  if (form.delivery === 'cdek') return 299
...
function validateForm() {
  ...
  if (form.delivery === 'courier' || form.delivery === 'cdek') {
    if (!form.city.trim()) { errors.city = 'Укажите город'; ok = false }
    if (!form.street.trim()) { errors.street = form.delivery === 'cdek' ? 'Укажите адрес ПВЗ' : 'Укажите улицу и дом'; ok = false }
  }
  ...
  if (form.payment === 'card') {
    ...
    if (!form.cardExpiry || form.cardExpiry.length < 5) { errors.cardExpiry = 'Введите срок действия (ММ/ГГ)'; ok = false }
...
function placeOrder() {
  ...
  saveOrder({
    items: cartItems.value.map(function(item) {
      return { productId: item.productId, name: item.product.name, size: item.size, qty: item.qty, price: effectivePrice(item.product) }
    }),
    ...
    <template v-if="step === 'cart'">
      ...
      <div v-else class="cart-layout">
          ...
          <div v-for="item in cartItems" :key="item.productId + '-' + item.size" class="cart-item">
            ...
            <div class="item-info">
              ...
              <div class="item-price-row">
                <span v-if="item.product.isSale && item.product.salePrice" class="item-price-old">{{ (item.product.price * item.qty).toLocaleString('ru-RU') }} ₽</span>
                <span class="item-price" :class="{ 'item-price--sale': item.product.isSale && item.product.salePrice }">{{ (effectivePrice(item.product) * item.qty).toLocaleString('ru-RU') }} ₽</span>
              </div>
              <div class="item-qty">
        ...
        <div class="cart-summary">
          ...
          <!-- Скидка по промокоду -->
          <div v-if="promoApplied" class="summary-row summary-discount">
            <span>Промокод {{ promoApplied }} {{ promoDiscount }}%</span>
            <span class="discount-val">− {{ promoDiscountAmount.toLocaleString('ru-RU') }} ₽</span>
          </div>
          ...
          <!-- Промокод (только для авторизованных) -->
          <template v-if="token && token !== 'admin'">
            <div class="summary-divider"></div>
            <div v-if="!promoApplied" class="promo-block">
              <div class="promo-row">
                <input v-model="promoInput" type="text" placeholder="ПРОМОКОД" class="promo-input" @input="promoError = ''" @keyup.enter="checkPromo" />
                <button class="promo-btn" @click="checkPromo">Применить</button>
              </div>
              <span v-if="promoError" class="promo-error">{{ promoError }}</span>
            </div>
            <div v-else class="promo-applied">
              <span>Промокод <b>{{ promoApplied }}</b> — −{{ promoDiscount }}%</span>
              <button class="promo-remove" @click="removePromo">✕</button>
            </div>
          </template>
          ...
          <template v-if="token && token !== 'admin'">
            <div class="summary-divider"></div>
            <div class="loyalty-progress-block">
              <div v-if="loyaltyLevel !== 'PLATINUM'" class="loyalty-progress-label">
                <span class="lp-level">{{ loyaltyLevel || 'Нет уровня' }}</span>
                <span class="lp-need">До {{ loyaltyProgress.next }}: ещё {{ loyaltyProgress.need.toLocaleString('ru-RU') }} ₽</span>
              </div>
              <div v-else class="loyalty-progress-label">
                <span class="lp-level">PLATINUM</span>
                <span class="lp-need">Максимальный уровень</span>
              </div>
              <div class="loyalty-bar-bg">
                <div class="loyalty-bar-fill" :style="{ width: Math.min(loyaltyProgress.pct, 100) + '%' }"></div>
              </div>
              <div class="lp-spent">Потрачено: {{ totalSpent.toLocaleString('ru-RU') }} ₽</div>
            </div>
          </template>
      ...
      <div class="checkout-layout">
        <div class="checkout-form">
          ...
          <div class="form-section">
            ...
            <div class="radio-group">
              ...
              <label class="radio-card" :class="{ 'radio-card--active': form.delivery === 'cdek' }">
                <input type="radio" v-model="form.delivery" value="cdek" />
                <div class="radio-body">
                  <span class="radio-label">СДЭК</span>
                  <span class="radio-desc">2–5 дней · 299 ₽</span>
                </div>
              </label>
            </div>
            ...
            <div v-if="form.delivery === 'cdek'" class="address-fields">
              <div class="form-row-2">
                <div class="form-field">
                  <label>Город *</label>
                  <input v-model="form.city" type="text" placeholder="Москва" :class="{ error: errors.city }" @input="delete errors.city" />
              ...
              <div class="form-field">
                ...
                <input v-model="form.street" type="text" placeholder="ул. Ленина, д. 1 (ПВЗ СДЭК)" :class="{ error: errors.street }" @input="delete errors.street" />
                <span v-if="errors.street" class="field-error">{{ errors.street }}</span>
              </div>
            </div>
          </div>
        ...
        <div class="checkout-summary">
          ...
          <div class="checkout-items">
            <div v-for="item in cartItems" :key="item.productId + '-' + item.size" class="checkout-item">
              ...
              <div class="checkout-item-info">
                ...
                <p class="checkout-item-price">{{ (effectivePrice(item.product) * item.qty).toLocaleString('ru-RU') }} ₽</p>
              </div>
            </div>
          </div>
          <div class="summary-divider"></div>
          ...
          <div class="summary-row">
            ...
            <span v-else-if="form.delivery === 'cdek'">299 ₽</span>
...
.item-price-row { display: flex; align-items: baseline; gap: 8px; margin-top: auto; }
.item-price-old { font-size: 12px; color: var(--gray-mid); text-decoration: line-through; }
.item-price--sale { color: #c0392b; font-weight: 600; }
...
/* Промокод */
.promo-block { margin: 4px 0; }
.promo-row { display: flex; gap: 6px; }
.promo-input { flex: 1; border: 1px solid var(--border); padding: 9px 11px; font-size: 11px; font-family: 'Montserrat', sans-serif; letter-spacing: 0.08em; text-transform: uppercase; color: var(--black); }
.promo-input:focus { outline: none; border-color: var(--black); }
.promo-btn { padding: 9px 14px; background: var(--black); color: var(--white); border: none; font-size: 10px; font-weight: 700; letter-spacing: 0.1em; text-transform: uppercase; cursor: pointer; font-family: 'Montserrat', sans-serif; white-space: nowrap; }
.promo-btn:hover { background: #333; }
.promo-error { font-size: 10px; color: #e53935; display: block; margin-top: 4px; }
.promo-applied { display: flex; align-items: center; justify-content: space-between; font-size: 12px; color: #2d7d2d; padding: 8px 0; }
.promo-remove { background: none; border: none; color: var(--gray-mid); cursor: pointer; font-size: 13px; padding: 0 2px; }
.promo-remove:hover { color: var(--black); }

/* Прогресс-бар лояльности */
.loyalty-progress-block { padding: 4px 0 8px; }
.loyalty-progress-label { display: flex; justify-content: space-between; align-items: baseline; margin-bottom: 7px; }
.lp-level { font-size: 10px; font-weight: 700; letter-spacing: 0.14em; text-transform: uppercase; color: var(--black); }
.lp-need { font-size: 10px; color: var(--gray-mid); }
.loyalty-bar-bg { height: 3px; background: var(--border); width: 100%; }
.loyalty-bar-fill { height: 3px; background: var(--black); transition: width .4s; }
.lp-spent { font-size: 10px; color: var(--gray-mid); margin-top: 5px; }
