...
<template>
  <div v-if="product" class="product-page">
    <div class="breadcrumb">
      ...
      <template v-if="route.query.from === 'sale'">
        <span class="bc-sep">/</span>
        <RouterLink :to="{ name: 'sale' }">Sale</RouterLink>
      </template>
      <template v-else-if="route.query.from === 'new'">
        <span class="bc-sep">/</span>
        <RouterLink :to="{ name: 'new-arrivals' }">Новинки</RouterLink>
      </template>
      <template v-else-if="route.query.from === 'collection'">
        <span class="bc-sep">/</span>
        <RouterLink :to="{ name: 'collection' }">Коллекция</RouterLink>
      </template>
      <template v-else-if="route.query.from === 'women'">
        <span class="bc-sep">/</span>
        <RouterLink :to="{ name: 'women' }">Женщинам</RouterLink>
      </template>
      <template v-else-if="route.query.from === 'men'">
        <span class="bc-sep">/</span>
        <RouterLink :to="{ name: 'men' }">Мужчинам</RouterLink>
      </template>
      <span class="bc-sep">/</span>
    ...
    <div class="product-layout">
      ...
      <div class="product-details">
        ...
        <div class="product-price-row">
          <span v-if="product.isSale && product.salePrice" class="product-price-old">{{ product.price.toLocaleString('ru-RU') }} ₽</span>
          <span class="product-price" :class="{ 'product-price--sale': product.isSale && product.salePrice }">{{ (product.isSale && product.salePrice ? product.salePrice : product.price).toLocaleString('ru-RU') }} ₽</span>
          <span v-if="product.isSale && product.salePrice" class="product-sale-badge">SALE −{{ Math.round((1 - product.salePrice / product.price) * 100) }}%</span>
        </div>
...
.product-sku { font-size: 10px; color: var(--gray-mid); letter-spacing: 0.12em; margin: 2px 0 8px; }
.product-price { font-size: 20px; font-weight: 400; color: var(--black); letter-spacing: 0.03em; }
.product-price-row { display: flex; align-items: baseline; gap: 10px; flex-wrap: wrap; margin-bottom: 2px; }
.product-price-old { font-size: 15px; color: var(--gray-mid); text-decoration: line-through; }
.product-price--sale { color: #c0392b; font-weight: 600; }
.product-sale-badge { font-size: 10px; font-weight: 700; letter-spacing: 0.12em; background: #c0392b; color: #fff; padding: 3px 8px; }
