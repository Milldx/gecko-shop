...
function isInStock(p) {
  ...
  for (let ki = 0; ki < keys.length; ki++) { if (p.stock[keys[ki]] > 0) return true }
  return false
}
...
const saleGoods = computed(function() {
  ...
  for (let gi = 0; gi < goods.value.length; gi++) {
    if (goods.value[gi].isSale) list.push(goods.value[gi])
  }
  ...
  if (sortBy.value === 'discount') list.sort(function(a, b) { return getDiscount(b) - getDiscount(a) })
  if (sortBy.value === 'price_asc') list.sort(function(a, b) { return (a.salePrice || a.price) - (b.salePrice || b.price) })
  if (sortBy.value === 'price_desc') list.sort(function(a, b) { return (b.salePrice || b.price) - (a.salePrice || a.price) })
...
const maxDiscount = computed(function() {
  let max = 0
  for (let si = 0; si < saleGoods.value.length; si++) {
    const d = getDiscount(saleGoods.value[si])
    if (d > max) max = d
  }
  return max
})
      ...
      <div v-for="p in saleGoods" :key="p.id" class="card" @click="goToProduct(p.id)">
        ...
        <div class="card-info">
          ...
          <div v-if="isInStock(p)" class="card-prices">
            ...
            <span class="card-price-sale">{{ (p.salePrice || p.price).toLocaleString('ru-RU') }} ₽</span>
          </div>
