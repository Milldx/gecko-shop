...
const filteredGoods = computed(function() {
  ...
  // Исключаем товары-новинки (они показываются в разделе НОВИНКИ, не в каталоге)
  list = list.filter(function(product) { return !product.isNew })

  // Исключаем товары SALE (они показываются только на странице SALE)
  list = list.filter(function(product) { return !product.isSale })

  // Исключаем товары коллекции (они показываются только на странице Коллекция)
  list = list.filter(function(product) { return !product.isCollection })

  // Поиск
  const q = searchQuery.value.trim().toLowerCase()
  if (q) list = list.filter(function(product) {
